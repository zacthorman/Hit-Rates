...
#
# Everywhere else a club's form is filtered to the competition being played,
# which is right: the Premier League is the best guide to the Premier League.
# Here it is wrong twice over. A club reaches the league phase having played
# eight European matches a season at most, and on matchday one it has played
# none at all, so the filter either returns nothing or returns a sample far
# too thin to price off. Worse, collect_events keeps the filtered set the
# moment it reaches five matches, so by midwinter one club in a tie can be
# judged on six European games while its opponent, one match short of the
# threshold, is judged on thirty-eight of everything. Two teams on the same
# page, scored on different evidence, with nothing on the page to say so.
#
# So for these, form is drawn from every competition the club has played.
# That sample is mostly domestic and it is not a like-for-like guide to
# European football. It is the honest best available, and it is marked as
# cross-league in the payload so the report can say so rather than quietly
# discarding it.
...
def build_fixture(
    ...
    entry = {
        "fixture": {
            ...
            # Set when the two clubs come from different domestic leagues, so
            # their form is all-competition by design rather than by accident.
    ...
    # No opponent adjustment across leagues, and no pretending otherwise.
    #
    # The ratings put every club on one scale by normalising within a pool of
    # sides that have played enough common opponents to be comparable, and two
    # clubs from different countries have played none. project_fixture would
    # refuse the pair anyway on MIN_PAIR_LINKS, which is the Hull City guard
    # doing exactly its job. The reason to stop earlier is the refit below: it
    # falls back to the competition most common in the two clubs' records,
    # which for a cross-league tie is one side's domestic league, and _in_pool
    # is satisfied by finding either club in it. That yields a fit containing
    # the home side and not the away side, and a projection built from it
    # would look ordinary and be meaningless. Fitting 36 clubs to reach that
    # also costs hours of requests.
    if adjust and cross_league:
        print("  no opponent adjustment: these clubs share no opposition to "
              "calibrate against")
        entry["ratingNotes"] = [
            "These clubs play in different leagues and have no opponents in "
            "common, so there is no shared scale to rate them on"
        ]
    elif adjust:
    ...
    # Not for a cross-league competition. The tier map is built by stacking
    # last season's final tables, and there is no table that ranks a Spanish
    # club against a German one. Asked for tid 7 it reads the previous
    # European season instead, which before 2024 was a four-team group table,
    # and everyone missing from it defaults to "bottom" -- so every opponent
    # in the competition is labelled the weakest there is. That is not a
    # degraded view, it is a confident wrong one, and it feeds projections.
    if tiers and unique_id in CROSS_LEAGUE_COMPETITIONS:
        print("  skipping the tier view: no table ranks these leagues against"
              " each other")
    elif tiers:
    ...
    if args.leagues:
        ...
        for name in wanted:
            ...
            try:
                ...
                # A competition with nothing on this week is not a fault. The
                # Champions League plays eight matchdays a season, so for most
                # of the year "no fixtures" is the correct answer and must not
                # stop the other seven leagues being published. Anything else
                # -- a name that does not resolve, a team list that would not
                # read, a fixture that produced no data at all -- is a real
                # failure and the run is not clean.
        ...
        if empty:
            print(f"Nothing on this week: {', '.join(empty)}")
        print("Now run: python make_index.py")

        # A competition that failed used to be a line in the middle of a
        # fourteen hour log and an exit code of zero, so update.py published a
        # site with a league quietly missing from it and called that success.
        # The run is only clean if everything asked for was built.
        if failed:
            raise SystemExit(
                f"Failed: {', '.join(failed)}. "
                f"{len(built)} of {len(wanted)} report(s) built; the others are "
                "missing from this run, and any older report for them is stale."
            )
        return
